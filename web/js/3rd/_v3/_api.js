...
(function() {
     ...
     */
    TShark.prototype.call = function(api, data, headers){
        data = data || {};
    ...
    function before(settings, el) {
        ...
        // Ajusta API
        var api     = str.split(' ')
            , extra = api.length == 4 ? api.pop() : false
            , map   = api.pop()
            , path  = api.join('.')
            , Func  = map.capitalize()
        ;
        ...
        if (tshark[map + '_before']) {
            if (!tshark[map + '_before'].call(tshark, el, settings)) return false;
        }
        ...
        settings.url = ('api/' + api.join('/') + tshark.api_map[map][1]).replace('{key}', extra); // + (extra ? '/' + extra : '');
    ...
    function callback(response) {
        ...
        if (func) {
            ...
            if (func == 'edit' || func == 'create'){
                onCallback('on', 'form', response);
            }

            // onCallback - data
            if (response['data']) {
                onCallback('on', 'data', response);
            }

            // onCallback
            onCallback('on', func, response);



            // onAfterCallback - gets
            if (func == 'list' || func == 'search' || func == 'get'){
                onCallback('onAfter', 'get', response);
            }

            // onAfterCallback - forms
            if (func == 'edit' || func == 'create'){
                onCallback('onAfter', 'form', response);
            }
            
            // onAfterCallback - save
            if (func == 'insert' || func == 'update'){
                onCallback('onAfter', 'save', response);
            }

            // onAfterCallback
            onAfterCallback('onAfter', func, response);
     ...
     *  onCallback - app    | assinatura: function(response, next) | this == app
     *  onCallback interno  | assinatura: function(response)       | this == tshark
     * @param prefix
     * @param func
     * @param response
     */
    function onCallback(prefix, func, response){
        var Func = func.capitalize();

        // onCallback - app | assinatura function(response, defCallback) | this == app
        if (app[prefix + Func]) {
            app[prefix + Func].call(app, response, function () {

                // Permite ao app chamar o onCallback padrão
                if (tshark[func + '_callback']) {
                    tshark[func + '_callback'].call(tshark, response);
                }
     ...
     * @param response
     */
    function onAfterCallback(prefix, func, response){
        var Func = func.capitalize();

        // onAfter no app | assinatura function(mod, response) | this == app
        if (app[prefix + Func]) {
            app[prefix + Func].call(app, response);
        }
     ...
     * @since 15/03/16
     */
    TShark.prototype.data_callback = function (response) {
     ...
     * @since 06/10/15
     */
    TShark.prototype.list_callback = function (response) {
